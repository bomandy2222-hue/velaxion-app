...
export default function App() {
  ...
  const saveToFirestore = async (nextForm, nextChecks, successText = "저장 완료!") => {
    if (!user) {
      setMessage("먼저 로그인해줘.");
      setMessageType("error");
      return;
  ...
  useEffect(() => {
    ...
    const timer = setTimeout(() => {
      saveToFirestore(form, checks, "자동 저장 완료!");
    }, 300);
  ...
  const handleSave = async () => {
    await saveToFirestore(form, checks, "저장 완료!");
  };
    ...
    <div style={styles.page}>
      <div style={styles.container}>
        <div style={styles.header}>
          <div>
            <h1 style={styles.title}>Velaxion 🚀</h1>
            <p style={styles.subtitle}>성장 기록과 실행 체크를 한 곳에서 관리해.</p>
          </div>
        </div>
